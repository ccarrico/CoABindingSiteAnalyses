...
filename_list_lines = file('Unique_NADP_Good_RMS_matches.txt').read().split("\n")[:-1]
LigandDict = {}
...
for filename in LigandDict:
    ...
    NADX_ligand_list = [filename + '/' + x for x in LigandDict[filename]]

    print filename, len(NADX_ligand_list)

    for ligand_fname in NADX_ligand_list:
        ...
        try:
            Ligand_Alignment_Trio = [ligand_set.get_atom_type('PA').Coords[0],
                                     ligand_set.get_atom_type('O3').Coords[0],
                                     ligand_set.get_atom_type('PN').Coords[0]]
        except IndexError:
            continue
